...
{
    private PackageReaderFactory $reader_factory;

    public function __construct(?PackageReaderFactory $reader_factory = null)
    {
        $this->reader_factory = $reader_factory ?? new PackageReaderFactory();
    }

    public function validate(string $package_path): ArchiveValidationResult
    {
        $errors = array();
        $manifest = array();
        $entry_count = 0;
        ...
        try {
            $reader = $this->reader_factory->create($package_path);
        ...
        foreach ($entries as $name) {
            $entry_count++;
            if (!PackagePathGuard::isSafe($name)) {
